...
const addCourse=async(req,res)=>{
    try {
        ...
        const newCourse=await courseModel.create({
            name,details,duration,brochureUrl,category,trainer,isPublished:true
        });
        const populatedCourse=await courseModel.findById(newCourse._id).populate("trainer").populate("category")
...
const updateCourse=async(req,res)=>{    
    try {
        ...
        const brochureUrl=result.url;
        const updatedCourse=await courseModel.findOneAndUpdate({_id:req.params.id},req.body,{new:true});
         const populatedCourse=await courseModel.findById(updatedCourse._id).populate("trainer").populate("category")
        return res.json(populatedCourse);
    } catch (error) {
